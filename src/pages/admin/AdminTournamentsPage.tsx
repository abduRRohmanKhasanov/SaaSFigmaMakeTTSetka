  ...
  return (
    <div className="relative pb-24">
          ...
          >
            <div className="flex items-start justify-between gap-3">
              <div className="flex-1 min-w-0">
                ...
                <p className="body-small text-on-surface-variant mt-1">{t.date} · {t.participantIds.length} участников</p>
              </div>
              <span className={cn("inline-flex items-center px-2 h-6 rounded-xs label-small shrink-0", STATUS_COLORS[t.status])}>
                {t.status}
              </span>
            </div>
          </button>
      ...
      <div className="fixed bottom-24 right-4 z-40">
        <Fab icon={<Plus size={24} />} extended label="Турнир" onClick={() => navigate("/admin/tournaments/new")} />
      </div>
    </div>
  );
}
